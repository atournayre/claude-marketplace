...
| `prompt-analyst` | [prompt](/plugins/prompt) | Analyse architecture + design DDD : explore le codebase, propose l'architecture technique et conçoit le modèle de domaine. Fusionne architect + designer. | Read, Grep, Glob, Bash |
...
| `prompt-implementer` | [prompt](/plugins/prompt) | Implémente le code ET écrit les tests selon le plan validé. Fusionne developer + tester. Accès écriture complet. | Read, Write, Edit, Grep, Glob, Bash |
